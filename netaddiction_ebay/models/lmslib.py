...
class UploadFile( FileTransferService ):
    ...
    def buildRequest( self, jobID, fileID, filename ):
        ...
        Args:
            ...
            filename(string): The filename( path) of the data file to be uploaded A STRIN
        '''
...
class GetJobs( BulkDataExchangeService ):
    ...
    def buildRequest(self, jobtype_list=None, jobstatus_list=None):
        '''
        ...
        if jobstatus_list:
            for jobstatus in jobstatus_list:
...
class DownloadFile( FileTransferService ):
    ...
    def _parse_response( self ):
        ...
        myzipfile = zipfile.ZipFile(out)
        for name in myzipfile.namelist():
            
            unzipped = myzipfile.open(name)
