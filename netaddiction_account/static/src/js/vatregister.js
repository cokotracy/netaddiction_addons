    ...
    var vatregister = Widget.extend({
        ...
        get_vatregister : function(e){
            ...
            new Model('stock.picking').call('get_picking_from_data',[year,month]).then(function(pickings){
                ...
                if($('#single_products').prop('checked')){
                    var varproducts = new products(null,pickings);
                    return varproducts.appendTo('#content_vatregister');
                }
      ...
      init : function(parent,pickings){
        ...
        $(pickings.refund).each(function(index,value){
            ...
            if(value.pid in picks['refund']){
                ...
                picks['refund'][value.pid]['total_price'] = picks['refund'][value.pid]['total_price'] + parseFloat(value.total_price)
                picks['refund'][value.pid]['price_tax'] = picks['refund'][value.pid]['price_tax'] + parseFloat(value.price_tax)
                picks['refund'][value.pid]['edizioni'] = picks['refund'][value.pid]['edizioni'] + parseFloat(value.edizioni)
        ...
        this.refund_price_tax = refund_price_tax;
        this.refund_edizioni = refund_edizioni;
