    ...
    def write(self, values):
        for product in self:
            ...
            if 'out_date' in values and values['out_date'] != product.out_date:
                log_line.sudo().create(log_line.create_tracking_values(product.out_date, values['out_date'], 'out_date', 'date', 'product.product', product.id, self.env.uid, object_name=product.name))
        ...
        if old_intax:
            for product in self:
                ...
                if product.id in old_intax:
                    log_line.sudo().create(log_line.create_tracking_values(old_intax[product.id], product.intax_price, 'intax_price', 'float', 'product.product', product.id, self.env.uid, object_name=product.name))
        return res
