            ...
            if pay and stock.sale_order_payment_method.id == cc_pay.id:
                ...
                if payment.journal_id.id == cc_pay.id:
                    if payment.state != 'posted' and payment.cc_status != 'commit':
                        try:
                            self.env['netaddiction.positivity.executor'].auth_and_check(payment.partner_id.id,payment.partner_id.email,payment.amount,payment.cc_token,stock.sale_id.id)
                        except Exception, e: 
                            error_stock.append(stock.id)
                            note.append(str(e))
