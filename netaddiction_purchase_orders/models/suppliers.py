...
class Suppliers(models.Model):
    ...
    def generate_monday_report(self):
        if self.supplier and len(self.parent_id) == 0:
            ...
            sale = products_preorder.get_sale_in_interval_date(date_start.strftime('%Y-%m-%d 00:00:00'), today.strftime('%Y-%m-%d 23:59:59'), count_refund=True, order_states=['sale', 'problem'])
            sale_all = products_preorder.get_sale_in_interval_date('1999-01-01', today.strftime('%Y-%m-%d 23:59:59'), count_refund=True, order_states=['sale', 'problem'])
    ...
    def get_products_in_stock(self):
        ...
        domain = [('location_id', '=', wh), ('history_ids.picking_id.partner_id', '=', self.id), ('company_id', '=', self.env.user.company_id.id)]
    ...
    def get_products_shipped(self, date_start, date_finish, count_refund=False):
        ...
        domain = [('date', '>=', date_start), ('date', '<=', date_finish), ('state', '=', 'done'), ('quant_ids.history_ids.picking_id.partner_id', '=', self.id),
        ...
        if count_refund:
            ...
            ('company_id', '=', self.env.user.company_id.id), ('quant_ids.history_ids.picking_id.partner_id', '=', self.id), ('location_id', '=', customer_stock), ('location_dest_id', 'in', [stock, difettati])]
