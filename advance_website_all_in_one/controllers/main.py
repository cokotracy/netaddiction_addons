...
class AdvanceCartSetting(WebsiteSale):
    ...
    def checkout(self, **post):
        ...
        if float(min_cart_value) > float(cart_total):
            return request.redirect(
                "/shop/cart?error_msg=%s"
                % _("Minimum cart amount is %s and your current order amount %s.")
    ...
    def get_loyalty_points(self, **post):
        ...
        config = (
            request.env["web.loyalty.setting"]
            .sudo()
            .search([("active", "=", True), ("issue_date", "<=", today_date), ("expiry_date", ">=", today_date)])
        )
        redeem_value = 0.0
        loyalty_amount = 0.0
    ...
    def redeem_loyalty_points(self, **post):
        ...
        if config:
            ...
            partner.sudo().write({"loyalty_points": partner.loyalty_points - int(post.get("entered_points"))})
    ...
    def cart_update_json(self, product_id, line_id=None, add_qty=None, set_qty=None, display=True):
        ...
        if so_line.discount_line and set_qty == 0:
            ...
            partner.sudo().write({"loyalty_points": partner.loyalty_points + so_line.redeem_points})
            so_line.order_id.sudo().write({"order_redeem_points": 0})
        ...
        value["website_sale.cart_lines"] = request.env["ir.ui.view"]._render_template(
            ...
            {"website_sale_order": order, "date": date.today(), "suggested_products": order._cart_accessories()},
        )
    ...
    def payment_validate(self, transaction_id=None, sale_order_id=None, **post):
        ...
        if tx.acquirer_id.provider == "cod":
            ...
            extra_fees_product = request.env["ir.model.data"].get_object_reference(
                "advance_website_all_in_one", "product_product_fees"
            )[1]
                            ...
                            order.partner_id.update(
                                {
                                    ...
                                    + order.order_line.price_unit * order.order_line.product_uom_qty
                                }
                            )
                        order.with_context(send_email=True).action_confirm()
                        request.website.sale_reset()
    ...
    def _apply_coupon(self, arg):
        ...
        order.write({"sale_coupon_id": coupon.id})
...
class PortalLoyaltyHistory(CustomerPortal):
    def _prepare_portal_layout_values(self):
        ...
        loyalty_count = LoyaltyHistory.sudo().search_count([("partner_id", "=", partner.id)])
        values.update(
            {
    ...
    def portal_my_loyalty_history(self, page=1, date_begin=None, date_end=None, sortby=None, **kw):
        ...
        # make pager
        pager = portal_pager(url="/my/loyalty/history", total=loyalty_count, page=page, step=self._items_per_page)
        # search the count to display, according to the pager data
        loyalty_points = LoyaltyHistory.sudo().search(
            domain, order=sort_loyalty, limit=self._items_per_page, offset=pager["offset"]
        )
        values.update(
            {
...
class WebsiteSale(http.Controller):
    ...
    def _get_compute_currency(self, pricelist, product=None):
        company = (
            product
            and product._get_current_company(pricelist=pricelist, website=request.website)
            or pricelist.company_id
            or request.website.company_id
        )
    ...
    def shop(self, page=0, category=None, abc=None, search="", ppg=False, **post):
        ...
        if setting.visitor_product_categ_ids:
            for c_id in setting.visitor_hide_product_categ_ids:
                visitor_hide_product_categ_ids = (
                    request.env["product.template"].sudo().search([("categ_id", "=", c_id.id)])
                )
        ...
        if products:
            ...
            filters = ProductFilter.search(
                [("filter_value_ids", "!=", False), ("filter_ids.product_tmpl_id", "in", selected_products.ids)]
            )
        ...
        if filter_group:
            grouped_tasks = [
                request.env["product.filter"].concat(*g) for k, g in groupbyelem(filters, itemgetter("group_id"))
            ]
        else:
    ...
    @http.route(
        ["""/shop/product/<model("product.template"):product>""", """/shop/product/<abc>"""],
        type="http",
        auth="public",
        website=True,
    )
    def product(self, product=None, abc=None, category="", search="", **kwargs):
        ...
        product_context = dict(
            request.env.context, quantity=add_qty, active_id=product.id, partner=request.env.user.partner_id
        )
        ...
        def compute_currency(price):
            return product.currency_id._convert(
                ...
                product._get_current_company(pricelist=pricelist, website=request.website),
                fields.Date.today(),
            )
        ...
        values = {
            ...
            "optional_product_ids": [
                p.with_context({"active_id": p.id}) for p in product.valid_product_template_attribute_line_ids
            ],
            "seo_url": "abcd",
        }
